...
// Based on the pa_kernels_QK_1014.cuh, modify 4-thread into 64-thread 
// loading the continuous K cache + BlockSize 16/64/256
...
__inline__ __device__ void _paged_attention_kernel_Jacob(
    ...
    const AttentionVariant* variant)
{
    ...
    for(int token_depth = 0; token_depth < TLOOP; token_depth++) // 4
    {
        for(int iter_16tk = 0; iter_16tk < ITERS_16TK; iter_16tk++) // 4
        {
            ...
            const int warp_token_offset = token_depth * 64 + iter_16tk * 16 + warpid * TOKEN_PER_WARP_FETCH;
    ...
    // [HIP] threadIdx=0, q_ptr=0x7ebcd5a00200, qhead_element=0, local_qhead_idx=0
    // [HIP] threadIdx=1, q_ptr=0x7ebcd5a00200, qhead_element=8, local_qhead_idx=0
    // [HIP] threadIdx=2, q_ptr=0x7ebcd5a00200, qhead_element=16, local_qhead_idx=0
    // [HIP] ...
    // [HIP] threadIdx=15, q_ptr=0x7ebcd5a00200, qhead_element=120, local_qhead_idx=0
    // [HIP] threadIdx=16, q_ptr=0x7ebcd5a00300, qhead_element=0, local_qhead_idx=1
            ...
            for(int head_loop = 0; head_loop < HEAD_LOOP; head_loop++) { // 1
                ...
                if ((local_mtp_qhead_idx < GQA_RATIO_MTP_PARALLEL) && (qhead_element < HEAD_SIZE)) {
                    ...
                    if constexpr (KV_DTYPE == vllm::Fp8KVCacheDataType::kAuto) {
                        ...
                        // uintptr_t byte_address0 = reinterpret_cast<uintptr_t>(&shared_logits[gqa_ratio_loop][head_loop][mtp][offset1][lane4id][local_qhead_idx][0]);
                        // uintptr_t byte_address1 = reinterpret_cast<uintptr_t>(&shared_logits[gqa_ratio_loop][head_loop][mtp][offset1][lane4id][local_qhead_idx][1]);
                        // uintptr_t shared_logits_base = reinterpret_cast<uintptr_t>(shared_logits);
                        // size_t relative_byte_offset0 = byte_address0 - shared_logits_base;
                        // size_t relative_byte_offset1 = byte_address1 - shared_logits_base;
                        // int bank_idx0 = (relative_byte_offset0 / 4) % 32;
    ...
    auto load_K_fragment = [&] __device__ (
        ...
        for (int head_loop = 0; head_loop < HEAD_LOOP; head_loop++) {
            for(int iter_16tk = 0; iter_16tk < ITERS_16TK; iter_16tk++){ // 4
                ...
                const int offset = kblock_number * kv_block_stride + kblock_offset * HEAD_SIZE + lane16id * CONTIGUOUS_KV_ELEMS_16B_LOAD;
                ...
                if constexpr (NT_KV_LOAD)
                    *reinterpret_cast<_B16x8*>(&Kbuffer_lds[buf_idx][token_row][cache_offset]) = 
                        load_ntmprl_16Byte(k_ptr_B16x8);
                else
                    *reinterpret_cast<_B16x8*>(&Kbuffer_lds[buf_idx][token_row][cache_offset]) = 
                        *k_ptr_B16x8;

                // if (threadIdx.x < 64 && iter_16tk==0  && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
        ...
        for (int head_loop = 0; head_loop < HEAD_LOOP; head_loop++) {
            for(int qkhe_depth = 0; qkhe_depth < QKHELOOP; qkhe_depth++){ // 4
                ...
                Kbuffer_reg[buf_idx][head_loop][qkhe_depth] = 
                    *reinterpret_cast<_B16x8*>(&Kbuffer_lds[buf_idx][row][col]);
                    // _B16x8{{
                    //     {static_cast<uint16_t>(NAN),1,1,1},{1,1,1,1}
                    // }};
                ...
                // if (threadIdx.x < 64 &&  blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
    ...
    const int qkout_token_idx = partition_start_token_idx + 16 * warpid + rowid * 4;
    ...
    for (int token_depth = 0; token_depth < TLOOP; token_depth++) { // 4
        ...
        // ALIBI
        const int local_token_idx = qkout_token_idx + token_depth * TOKENS_PER_WARP;
    ...
    // Note: A workgroup has 4 wavefronts. Save qk_max and exp_sum fromn each wavefront into LDS
        ...
        for(int mtp = 0; mtp < mtp_loop; mtp++) {
            for(int gqa_ratio_loop = 0; gqa_ratio_loop < GQA_RATIO_LOOP; gqa_ratio_loop++) {  
                ...
                if (threadIdx.x < 256 &&  blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
                    printf("[qk_max + exp_sum] threadIdx=%3d, qk_max_offset=%d, exp_sum_offset=%3d\n",
