    ...
    const AttentionVariant* variant)
{
    ...
    // Jacob: mtp_loop=1, GQA_RATIO_LOOP=1, HEAD_LOOP=1
    // Jacob: SGLang doesn't set MTP in aiter_backend.py so using aiter default value = 1
        ...
        for(int gqa_ratio_loop = 0; gqa_ratio_loop < GQA_RATIO_LOOP; gqa_ratio_loop++) {
            ...
            for(int head_loop = 0; head_loop < HEAD_LOOP; head_loop++) { 
                ...
                if ((local_mtp_qhead_idx < GQA_RATIO_MTP_PARALLEL) && (qhead_element < HEAD_SIZE)) {
                    ...
                    _B16x8 tmp = *q_fetch_ptr_16B; // Jacob: [Large latency]

                    if constexpr (KV_DTYPE == vllm::Fp8KVCacheDataType::kAuto) {
                        const int offset1 =
                            lane16id /
                            4;  // 16 contiguous chunks of head elems are spread across 4x4lanes
                        shared_logits[gqa_ratio_loop][head_loop][mtp][offset1][lane4id][local_qhead_idx][0] = tmp.xy[0];  // Jacob: [Large latency]
    ...
    // Jacob: Place this code segment "fetch K block" after loading K and hope to 
    // Jacob: hide syncthreads latency 
    // Jacob: TLOOP=4
    // Jacob: Large latency when batch size is large.
    for(int token_depth = 0; token_depth < TLOOP; token_depth++)
    {
        ...
        kphysical_block_number[token_depth] = block_table_seq[kblock_idx];  // Jacob: [Large latency]
        __builtin_amdgcn_sched_group_barrier(0x020, 1, 0); // VMEM read
        __builtin_amdgcn_sched_group_barrier(0x002, 5, 0); // VALU
        // __builtin_amdgcn_sched_barrier(0); // Prevent ops from crossing the interation
    }
    __syncthreads();

    // Jacob: QKHELOOP=4, QK_SIZE_RATIO=1, i=2, mtp_loop=1, GQA_RATIO_LOOP=1, HEAD_LOOP=1
    // Jacob: compiler unroll the 8 iteration loop and merge into 4 LDS instr as Qlocal[...].xy[i] are continuous
    // if (threadIdx.x == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
    //     printf("sizeof(scalar_t) = %d, sizeof(cache_t) = %d\n", sizeof(scalar_t), sizeof(cache_t) );
    //     printf("QKHELOOP = %d, QK_SIZE_RATIO = %d, HEAD_LOOP=%d\n", QKHELOOP, QK_SIZE_RATIO, HEAD_LOOP);  
    // }
    ...
    // Jacob: Load global data k_ptr to register Klocal and do mfma
    ...
    floatx4 d_out[GQA_RATIO_LOOP][MTP_PER_THREAD][TLOOP]; // QK result
    for(int token_depth = 0; token_depth < TLOOP; token_depth++)
    {
        ...
        for(int qkhe_depth = 0; qkhe_depth < QKHELOOP; qkhe_depth++)
        {
            for(int head_loop = 0; head_loop < HEAD_LOOP; head_loop++) {
                ...
                {
                    Klocal[head_loop][token_depth][qkhe_depth] = *k_fetch_ptr_16B; // Jacob: [Large latency]
                    // __builtin_amdgcn_sched_group_barrier(0x020, 1, 0); // VMEM read
        ...
        // qk mfma
        const int qkout_token_idx = partition_start_token_idx + TOKENS_PER_WARP * warpid + rowid * 4;  // ALIBI
        const int local_token_idx = qkout_token_idx + token_depth * 16; // ALIBI
        const int alibi_offset = local_token_idx - context_len + 1;     // ALIBI
        for (int gqa_ratio_loop = 0; gqa_ratio_loop < GQA_RATIO_LOOP; gqa_ratio_loop++) {
