    ...
    const AttentionVariant* variant)
{
    ...
    // Jacob: q_stride = GQA_RATIO * HEAD_DIM = 6*128=768
    //   each thread local 8 data, so 16 threads can load the full 128 head_dim of Q
    ...
    for(int token_depth = 0; token_depth < TLOOP; token_depth++) // 4, 4 thread load 4 token
    {
        ...
        for(int qkhe_depth = 0; qkhe_depth < QKHELOOP; qkhe_depth++) // 4, each thread load 16byte, so 4t load 64B, 4 iter load 256 Byte which match head_dim=128 elems(256B)
        {
            for(int head_loop = 0; head_loop < HEAD_LOOP; head_loop++) {
                ...
                // if(threadIdx.x == 0 && token_depth==0 && 
                //     blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0){
                //     printf("[Load K cache] threadIdx=%3d, qkhe_depth=%3d, "
                //            "kblock_number=%5ld, kphysical_block_offset=%2d, "
                //            "addr-base=%ld, val=%f\n",
                //         threadIdx.x, qkhe_depth,
                //         kblock_number, kphysical_block_offset, 
                //         k_fetch_ptr-k_cache, __bfloat162float(*k_fetch_ptr));
                // }
            }
        }
    }
    ...
    for (int mtp = 0; mtp < mtp_loop; mtp++) {
        for (int token_depth = 0; token_depth < TLOOP; token_depth++) {
            for (int gqa_ratio_loop = 0; gqa_ratio_loop < GQA_RATIO_LOOP; gqa_ratio_loop++) {
                ...
                //     printf("[check d_out] threadIdx.x=%d, token_depth=%d, d_out=%f,%f %f %f \n",
                //         threadIdx.x, token_depth, data[0], data[1], data[2], data[3]); 
                //     for(int y=0; y<QKHELOOP; ++y){
                //         printf("token_depth=%d, QKHELOOP=%d, k cache=",token_depth, y);
                //         for(int x=0; x<2; ++x)
                //         {
                //             _B16x4 kdata = Klocal[0][token_depth][y].xy[x];
                //             uint16_t v0 = kdata[0];
                //             uint16_t v1 = kdata[1];
                //             uint16_t v2 = kdata[2];
                //             uint16_t v3 = kdata[3];
